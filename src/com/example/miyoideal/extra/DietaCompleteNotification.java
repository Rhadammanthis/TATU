...
public class DietaCompleteNotification extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent){
		...
		Log.d("service", "Dia actual = " + String.valueOf(dia_actual));
		Log.d("service", "Dia inicial = " + String.valueOf(dia_inicial));
		Log.d("service", "(dia_actual - dia_inicial) >= (duracion - 1) => " + String.valueOf((dia_actual - dia_inicial) >= (duracion - 1)));

		if(true)//(dia_actual - dia_inicial) >= (duracion - 1))
		{
			Calendar currentDate = Calendar.getInstance();
			Calendar fixedDate = Calendar.getInstance();
			fixedDate.set(Calendar.HOUR_OF_DAY, 9);
			int hour = c.get(Calendar.HOUR_OF_DAY);
			if(true)//true)//currentDate.get(Calendar.HOUR_OF_DAY) >= fixedDate.get(Calendar.HOUR_OF_DAY))
			{
	...
	public void setAlarm(Context context, String duracion, String init_day){
		...
		alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, cal.getTimeInMillis(), 180000*1000, alarmIntent);
	}
